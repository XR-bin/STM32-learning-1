...
uint8_t W25Q128_Read_SR(uint8_t serial)
{
    ...
    SPI_CS_L;                   /* 使能片选 */
    SPI1_RS_Byte(command);      /* 发送读寄存器命令 */
    byte = SPI1_RS_Byte(0Xff);  /* 读取一个字节 */
    SPI_CS_H;                   /* 失能片选 */
...
{
    SPI_CS_L;
    SPI1_RS_Byte(FLASH_WriteEnable);
    SPI_CS_H;
}
...
{
    SPI1_RS_Byte((uint8_t)((address)>>16));     /* 发送 bit23 ~ bit16 地址 */
    SPI1_RS_Byte((uint8_t)((address)>>8));      /* 发送 bit15 ~ bit8  地址 */
    SPI1_RS_Byte((uint8_t)address);             /* 发送 bit7  ~ bit0  地址 */
}
...
{
    SPI_CS_L;                       /* 片选使能 */
    SPI1_RS_Byte(FLASH_ReadData);   /* 发读数据指令 */
    W25Q128_Send_Address(address);  /* 发送地址 */

    /* 收数据 */
    while(datalen)
    {
        *arr = SPI1_RS_Byte(0xff);
        arr++;
        datalen--;
    }
...
void W25Q128_Page_Write(uint32_t address, uint32_t datalen, uint8_t *arr)
{
    ...
    SPI_CS_L;                        /* 片选使能 */
    SPI1_RS_Byte(FLASH_PageProgram); /* 写页指令 */
    W25Q128_Send_Address(address);   /* 发送地址 */
...
void W25Q128_Sector_Erase(uint32_t address)
{
    ...
    /* 擦除 */
    SPI_CS_L;                        /* 片选使能 */
    SPI1_RS_Byte(FLASH_SectorErase); /* 扇区擦除指令 */
    W25Q128_Send_Address(address);   /* 发送地址 */
    SPI_CS_H;                        /* 片选失能 */
...
void W25Q128_Block_Erase(uint32_t address)
{
    ...
    /* 擦除 */
    SPI_CS_L;                        /* 片选使能 */
    SPI1_RS_Byte(FLASH_BlockErase);  /* 块区擦除指令 */
    W25Q128_Send_Address(address);   /* 发送地址 */
    SPI_CS_H;                        /* 片选失能 */
    W25Q128_Wait_Busy();             /* 等待擦除完成 */
}
...
void W25Q128_Chip_Erase(void)
{
    ...
    /* 擦除 */
    SPI_CS_L;                        /* 片选使能 */
    SPI1_RS_Byte(FLASH_ChipErase);   /* 芯片擦除 */
    SPI_CS_H;                        /* 片选失能 */
    W25Q128_Wait_Busy();             /* 等待擦除完成 */
}
