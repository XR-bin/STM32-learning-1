...
void OLED_IO_Init(void)
{
  ...
  //端口模式寄存器
  GPIOF->MODER &= ~((3<<2*0)|(3<<2*1)|(3<<2*2)|(3<<2*3));
  GPIOF->MODER |= (1<<2*0)|(1<<2*1)|(1<<2*2)|(1<<2*3);
  
  //端口类型寄存器
  GPIOF->OTYPER &= ~((1<<0)|(1<<1)|(1<<2)|(1<<3));
  
  //端口输出速率寄存器
  GPIOF->OSPEEDR &= ~((3<<2*0)|(3<<2*1)|(3<<2*2)|(3<<2*3));
  GPIOF->OSPEEDR |= ((3<<2*0)|(3<<2*1)|(3<<2*2)|(3<<2*3));
}
...
void OLED_Clear(void)
{
  ...
  for(i=0;i<8;i++)
  {
    ...
    for(j=0;j<128;j++) OLED_WR_Byte(0,OLED_DATA); //一个字节控制8行1列
  }
}
