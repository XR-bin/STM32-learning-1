...
#define _OLED_H
  ...
  #define  OLED_DC_L   GPIOB->ODR &= ~(1<<12)  //拉低DC，选择命令
  #define  OLED_DC_H   GPIOB->ODR |= (1<<12)   //拉高DC，选择数据
  //CLK  时钟线
  #define  OLED_CLK_L   GPIOB->ODR &= ~(1<<13)   //拉低CLK
  #define  OLED_CLK_H   GPIOB->ODR |= (1<<13)    //拉高CLK 
  //RST  硬复位OLED
  #define  OLED_RST_L   GPIOB->ODR &= ~(1<<14)   //拉低RST
  #define  OLED_RST_H   GPIOB->ODR |= (1<<14)    //拉高RST 
  //MOSI 主输出数据线
  #define  OLED_MOSI_L   GPIOB->ODR &= ~(1<<15)   //拉低MOSI
  #define  OLED_MOSI_H   GPIOB->ODR |= (1<<15)    //拉高MOSI
